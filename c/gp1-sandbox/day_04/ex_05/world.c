...
void add_entity(void* p_entity, t_explosion_event_callback explosion_callback)
{
	...
	the_world->entities->explosion_callback = explosion_callback;	//.explosion_callback is the function pointer
...
void delete_world()
{
	...
	{
		free(the_world->entities->entity[i]);
	}*/
...
void send_explosion_event(void* explosion_sender, char const* explosion_name, t_position const* explosion_position)
{
	...
	crate_explosion_event(explosion_sender, explosion_name, explosion_position);
	bomberman_explosion_event(explosion_sender, explosion_name, explosion_position);
	barrel_explosion_event(explosion_sender, explosion_name, explosion_position);
}
